...
$(function ($, window, document, undefined) {
  ...
  Plugin.prototype.init = function () {
    ...
    _$this.wrap('<div class=\'sp_wrapper\'><div class=\'sp_table\'>'),
      ...
      _$this.closest('.sp_wrapper').find('.sp_navigator')
        .append('<span><a class="sp_paginate sp_previous" href="">上一頁<a/><a class="sp_paginate sp_next" href="">下一頁<a/></span>')

    var paginate = function (start, size) {
      var tableRows = $(_$this).find('tbody tr'), end = start + size
      tableRows.hide(), tableRows.slice(start, end).show(), $(_$this).closest('.sp_wrapper').find('.sp_navigator .sp_paginate').show(),
      tableRows.eq(0).is(':visible') && $(_$this).closest('.sp_wrapper').find('.sp_navigator .sp_previous').hide(),
      tableRows.eq(tableRows.length - 1).is(':visible') && $(_$this).closest('.sp_wrapper').find('.sp_navigator .sp_next').hide()
    }

    _$this.find('th').on('click', function () {
      $(_$this).find('th').not($(this)).removeClass('sp_sorted_asc sp_sorted_desc'),
        $(this).hasClass('sp_sorted_asc') || $(this).hasClass('sp_sorted_desc') ? $(this).toggleClass('sp_sorted_asc sp_sorted_desc')
          : $(this).addClass('sp_sorted_asc'), $(_$this).find('tbody tr').show(), sortTable($(_$this), $(this).index(), $(this).hasClass('sp_sorted_asc') ? 'asc' : 'desc'),
      ...
      _$this.closest('.sp_wrapper').find('.sp_navigator').on('click', '.sp_paginate', function (e) {
        ...
        $(this).hasClass('sp_next') ? tmpRec += _opts.pageSize : tmpRec -= _opts.pageSize, tmpRec < 0
        || tmpRec > tableRows.length
        || ($(_$this).find('tbody').attr('data-firstRecord', tmpRec), paginate(tmpRec, 10))
      }),
